...
```{r}
knitr::include_graphics("figures/LOFcode.png")
```


From the code snippet above, it can be seen that: 
...
![LOF result visualisation](figures/LOFvis.png)

The above graph is the visualization of the Local Outlier Factor Model for the metric_contract_awarded table. The Blue values are Non anomalous while the red coloured data points are the anomalous ones. The Above graph is faceted by DTP_group which is one of the four important aggregations. 
...
![Visualising result after combinig models](figures/Combine.png)

From the graph above, we can be observe anomalies common in both the models, with contamination factor equal to 0.025 or 2.5%.
